"""
Code taken straight from deeplabcut and slightly repurposed
...
class MainFrame(wx.Frame):
    ...
    def browseDir(self, event):
        ...
        if os.path.isfile(self.dataname):
            ...
            if instruction == 4:
                ...
                """
                textBox = wx.TextEntryDialog(self, "Select the likelihood threshold", caption="Enter the threshold",
                                             value="0.1")
                textBox.ShowModal()
    ...
    def saveDataSet(self, event):
        ...
        #     DataCombined.to_hdf(os.path.join(self.dir, 'CollectedData_' + self.humanscorer + '.h5'),
        #                         key='df_with_missing', mode='w')
        #     DataCombined.to_csv(os.path.join(self.dir, 'CollectedData_' + self.humanscorer + '.csv'))
        # else:
        #     self.Dataframe.sort_index(inplace=True)
        #     self.Dataframe.to_hdf(os.path.join(self.dir, 'CollectedData_' + self.humanscorer + '.h5'),
        #                           key='df_with_missing', mode='w')
        ...
        #
        # nextFilemsg = wx.MessageBox('File saved. Do you want to refine another file?', 'Repeat?',
    ...
    def plot(self, im):
        ...
        for bpindex, bp in enumerate(self.bodyparts):
            ...
            if not ('CollectedData_' in self.fileName) and self.likelihood < self.threshold:
                circle = [patches.Circle((self.points[0], self.points[1]), radius=self.markerSize, facecolor='None',
                                         edgecolor=color)]
            else:
                circle = [patches.Circle((self.points[0], self.points[1]), radius=self.markerSize, fc=color,
                                         alpha=self.alpha)]
